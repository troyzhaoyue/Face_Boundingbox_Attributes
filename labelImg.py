...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None):
        ...
        listLayout.addWidget(self.illuminationButton0, 7, 0)
        listLayout.addWidget(self.illuminationButton1, 7, 1)
        listLayout.addWidget(self.illuminationButton2, 7, 2)
        listLayout.addWidget(self.illuminationButton3, 7, 3)
        listLayout.addWidget(self.illuminationButton4, 7, 4)

        self.occlusionlabel = QLabel()
        self.occlusionlabel.setText('occlusion:')
        listLayout.addWidget(self.occlusionlabel, 8, 0)
        self.occlusiongroup = QButtonGroup()
        self.occlusiongroup.exclusive()
        self.occlusionButton0 = QCheckBox(u'No occ')
        self.occlusionButton0.setChecked(False)
        self.occlusionButton0.stateChanged.connect(self.btnstate_no_occlusion)
        self.occlusionButton1 = QCheckBox(u'Partial(1/4)')
        self.occlusionButton1.setChecked(False)
        self.occlusionButton1.stateChanged.connect(self.btnstate_partial)
        self.occlusionButton2 = QCheckBox(u'Heavy(1/2)')
        self.occlusionButton2.setChecked(False)
        self.occlusionButton2.stateChanged.connect(self.btnstate_heavy)
        self.occlusionButton3 = QCheckBox(u'Sunglasses')
        self.occlusionButton3.setChecked(False)
        self.occlusionButton3.stateChanged.connect(self.btnstate_sunglasses)
        ...
        listLayout.addWidget(self.occlusionButton0, 9, 0)
        listLayout.addWidget(self.occlusionButton1, 9, 1)
        listLayout.addWidget(self.occlusionButton2, 9, 2)
        listLayout.addWidget(self.occlusionButton3, 9, 3)

        self.yawlabel = QLabel()
        self.yawlabel.setText('yaw:')
        listLayout.addWidget(self.yawlabel, 10, 0)
        ...
        listLayout.addWidget(self.yawButton0, 11, 0)
        listLayout.addWidget(self.yawButton1, 11, 1)
        listLayout.addWidget(self.yawButton2, 11, 2)

        self.rolllabel = QLabel()
        self.rolllabel.setText('roll:')
        listLayout.addWidget(self.rolllabel, 12, 0)
        ...
        listLayout.addWidget(self.rollButton0, 13, 0)
        listLayout.addWidget(self.rollButton1, 13, 1)
        listLayout.addWidget(self.rollButton2, 13, 2)

        self.pitchlabel = QLabel()
        self.pitchlabel.setText('pitch:')
        listLayout.addWidget(self.pitchlabel, 14, 0)
        ...
        listLayout.addWidget(self.pitchButton0, 15, 0)
        listLayout.addWidget(self.pitchButton1, 15, 1)
        listLayout.addWidget(self.pitchButton2, 15, 2)
        listLayout.addWidget(self.pitchButton3, 15, 3)
        listLayout.addWidget(self.pitchButton4, 15, 4)
        ...
        listLayout.addWidget(self.ageButton,16,0)
        listLayout.addWidget(useDefaultLabelContainer,16,1)
        ...
        create = action('Create\nRectBox', self.createShape,
                        'F', 'new', u'Draw a new Box', enabled=False)
        ...
        Shape.no_occlusion = self.no_occlusion
        Shape.partial = self.partial
        Shape.heavy = self.heavy
        Shape.sunglasses = self.sunglasses
    ...
    def loadLabels(self, shapes):
        ...
        for label, points, line_color, fill_color, \
            ...
            shape = Shape(label=label,
                          ...
                          no_occlusion=no_occlusion, partial=partial, heavy=heavy, sunglasses=sunglasses,
    ...
    def saveLabels(self, annotationFilePath):
        ...
        def format_shape(s):
            return dict(label=s.label,
                       ...
                       # add chris
                        ismale = s.ismale, isfemale = s.isfemale,
                        age = s.age,
                        ...
                        no_occlusion=s.no_occlusion, partial=s.partial, heavy=s.heavy, sunglasses=s.sunglasses,
    ...
    def printthisshape(self,shape):
        ...
        print "occlusion:",
        if shape.no_occlusion:
            print "norm occlusion"
        elif shape.partial:
            print "partial"
        elif shape.heavy:
            print "heavy"
    ...
    def btnstate_no_occlusion(self, item=None):
        if not self.canvas.editing():
            return
        item = self.currentItem()
        if not item:  # If not selected Item, take the first one
            item = self.labelList.item(self.labelList.count() - 1)
        no_occlusion = self.occlusionButton0.isChecked()
        try:
            shape = self.itemsToShapes[item]
            if no_occlusion != shape.no_occlusion:
                shape.no_occlusion = no_occlusion
                self.setDirty()
            else:  # User probably changed item visibility
                self.canvas.setShapeVisible(shape, item.checkState() == Qt.Checked)
        except:
    ...
    def btnstate_partial(self, item=None):
        if not self.canvas.editing():
            return
        item = self.currentItem()
        if not item:  # If not selected Item, take the first one
            item = self.labelList.item(self.labelList.count() - 1)
        partial = self.occlusionButton1.isChecked()
        try:
            shape = self.itemsToShapes[item]
            if partial != shape.partial:
    ...
    def btnstate_heavy(self, item=None):
        if not self.canvas.editing():
            return
        item = self.currentItem()
        if not item:  # If not selected Item, take the first one
            item = self.labelList.item(self.labelList.count() - 1)
        heavy = self.occlusionButton2.isChecked()
        try:
            shape = self.itemsToShapes[item]
            if heavy != shape.heavy:
                shape.heavy = heavy
                self.setDirty()
            else:  # User probably changed item visibility
                self.canvas.setShapeVisible(shape, item.checkState() == Qt.Checked)
    ...
    def btnstate_sunglasses(self, item=None):
        if not self.canvas.editing():
            return
        item = self.currentItem()
        if not item:  # If not selected Item, take the first one
            item = self.labelList.item(self.labelList.count() - 1)
        sunglasses = self.occlusionButton3.isChecked()
        try:
            shape = self.itemsToShapes[item]
            if sunglasses != shape.sunglasses:
                shape.sunglasses = sunglasses
                self.setDirty()
            else:  # User probably changed item visibility
                self.canvas.setShapeVisible(shape, item.checkState() == Qt.Checked)
        except:
